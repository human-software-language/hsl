...
class StepByStepPlanSignature(dspy.Signature):
    ...
    task_description = dspy.InputField(
        desc="""Detailed task description with algorithm how predict expected_output and step_by_step_plan""",
    )
    expected_output = dspy.OutputField(
        # desc="""expected output in format `table: element1, element2, ...`""",
    )
...
def main():
    ...
    # Discover
    self_discover = BrowserDiscover(model="gpt-4-turbo-preview")
